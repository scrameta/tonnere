...
architecture vhdl of tonnere is
	...
	signal SIO_CLOCK_OUT_INT : std_logic;
	signal SIO_COMMAND_INT : std_logic;
	signal SIO_DATA_OUT_INT : std_logic;
...
--    SDRAM_WRITE_ENABLE <= not(FSMC_NWE);
    ...
    FSMC_D <= (others=>'1') when FSMC_NOE='0' else (others=>'Z');
    FSMC_NWAIT <= '1';
...
--    FSMC_NWAIT <= '1';       --Do not wait
    ...
    -- SIO (Serial I/O)
    SIO_DATA_IN     <= 'Z';
    SIO_DATA_OUT    <= '0' when SIO_DATA_OUT_INT='0' else 'Z';
    SIO_CLOCK_IN    <= 'Z';
    SIO_CLOCK_OUT   <= '0' when SIO_CLOCK_OUT_INT='0' else 'Z';
    SIO_COMMAND     <= '0' when SIO_COMMAND_INT='0' else 'Z';
    SIO_PROCEED     <= 'Z';
    SIO_INTERRUPT   <= 'Z';
    SIO_MOTOR       <= 'Z';
...
hello_world : ENTITY work.atari800core_simple_sdram
	...
	(
		...
		-- SIO
		SIO_COMMAND => SIO_COMMAND_INT,
		SIO_RXD => SIO_DATA_IN,
		SIO_TXD => SIO_DATA_OUT_INT,
		SIO_CLOCKOUT => SIO_CLOCK_OUT_INT,
		...
		HALT => '0',
		TURBO_VBLANK_ONLY => '0',
		THROTTLE_COUNT_6502 => "000001", --std_logic_vector(to_unsigned(32-1,6)),
